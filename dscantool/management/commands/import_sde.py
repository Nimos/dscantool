...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        newGroups = []
        try:
            file = open("eve_data/groupIDs.yaml", encoding="utf8")
        ...
        newTypes = []
        try:
            file = open("eve_data/typeIDs.yaml", encoding="utf8")
            types = yaml.load(file)

            for g in types:
                print(g, end="\r")
                if (types[g]["name"].get("en", False)):
                    newTypes.append(InvType(typeID=g, typeName=types[g]["name"].get("en").encode("utf8"), group_id=types[g]["groupID"]))
